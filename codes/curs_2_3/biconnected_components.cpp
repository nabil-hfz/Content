...
//    Articulation Points (highlighted in above figure). As soon as an Articulation Point u is found,
...
//    If there is no Articulation Point in graph, then graph is biconnected and
//    so there will be one biconnected component which is the graph itself.
#define NIL -1
using namespace std;
int cCount = 0;
...
// low[] -- >> earliest visited vertex (the vertex with minimum discovery time) that can be reached from subtree
...
int main() {
    ...
    g.addEdge(11, 10);
    g.BCC();
    cout << "Above are " << cCount << " biconnected components in graph";
    return 0;
}
